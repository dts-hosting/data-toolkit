...
class CreateUpdateRecordsPage {
  ...
  async navigateTo(toolkitUrl) {
    ...
    // Navigate to the Create or Update Records page
    await this.page.getByRole('link', { name: 'Create or Update Records' }).click();
    
    // Verify that we are on the correct page
    await expect(this.page.getByText('New Create or Update Records')).toBeVisible();
  }

  async createUpdateRecords(file, statusExpected, countExpected) {
    // Locate the opetion of Collection Object 
    const option = await this.page.locator('option:has-text("collectionobject")').textContent();
    // Select the Option 
...
module.exports = CreateUpdateRecordsPage;