...
export class CheckMediaDerivativesPage {
  ...
  async navigateTo(toolkitUrl) {
    ...
    // Navigate to the Export Record IDs page
    await this.page.getByRole('link', { name: 'Export Record IDs' }).click();
    
    // Verify that we are on the correct page
    await expect(this.page.getByText('New Export Record IDs')).toBeVisible();
  }
