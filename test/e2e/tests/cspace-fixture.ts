...
export class CSpaceFixture {
  ...
  async goto(linkToClick: string, titleToValidate: string, local_page = this.page) {
  ...
  async fillFormAndSubmit(optionTextToSearch: string, file: string, local_page = this.page) {
  ...
  async searchItem(itemToSearch: string, itemType: string = "All Records", local_page = this.page) {
  ...
  async searchByIdAndTitle(itemId: string, itemTitle: string, local_page = this.page) {
    // Search for the created task
    
    await local_page.goto(this.baseURL+'cspace/anthro/search/collectionobject');
    await local_page.locator('.cspace-ui-TitleBar--common').getByText('Search').waitFor();

    await local_page.locator('header').filter({ hasText: 'of the following conditions' }).getByRole('textbox').click();
    ...
    await local_page.locator('div:nth-child(2) > .cspace-input-LineInput--embedded').first().click();
    await local_page.locator('div:nth-child(2) > .cspace-input-LineInput--embedded').first().fill(itemId);
    await local_page.locator('li:nth-child(7) > .cspace-ui-FieldConditionInput--normal > .cspace-input-DropdownMenuInput--common > .cspace-input-LineInput--normal').click();
    await local_page.getByRole('option', { name: 'matches' }).click();
    await local_page.locator('li:nth-child(7) > .cspace-ui-FieldConditionInput--normal > div:nth-child(3) > .cspace-input-RepeatingInput--normal > div > div > div:nth-child(2) > .cspace-input-LineInput--embedded').click();
    await local_page.locator('li:nth-child(7) > .cspace-ui-FieldConditionInput--normal > div:nth-child(3) > .cspace-input-RepeatingInput--normal > div > div > div:nth-child(2) > .cspace-input-LineInput--embedded').fill(itemTitle);
    await local_page.getByRole('contentinfo').filter({ hasText: 'SearchClear' }).locator('button[name="search"]').click();
  ...
  async fetchRelatedObjects (itemToSearch: string, local_page = this.page) {
    const relatedObjects = await local_page.getByRole('button', { name: 'Related Objects:' });
    await relatedObjects.click();
    return local_page.locator('.cspace-ui-SearchResultTable--common').locator('.ReactVirtualized__Grid__innerScrollContainer').count();
  }
}
